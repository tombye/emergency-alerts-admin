...
class Service(JSONModel, SortByNameMixin):
    ...
    def is_folder_in_use(self, template_folder_id):
        return any(
            self.get_templates(template_type='all', template_folder_id=template_folder_id) +
            [folder for folder in self.all_template_folders if folder['parent_id'] == template_folder_id]
        )
